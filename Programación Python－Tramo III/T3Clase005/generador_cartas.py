...
def generador_cartas(cantidad, tapada=False, color=None, rango_numeros=None):
    producidas = 0
    while producidas < cantidad:
        num = random.randint(1, 13)
        palo = random.randint(1, 4)
        ...
        # Verificar si la carta cumple con el color especificado
        if color:
            if color == "rojo" and palo not in (1, 2):  # 1: Corazón, 2: Diamante
                carta_valida = False
            elif color == "negro" and palo not in (3, 4):  # 3: Trébol, 4: Pica
                carta_valida = False
        
        # Verificar si la carta cumple con el rango de números especificado
        if rango_numeros and (num < rango_numeros[0] or num > rango_numeros[1]):
            carta_valida = False
        
        # Si la carta es válida según los criterios, la producimos y aumentamos el contador
        if carta_valida:
            producidas += 1
            yield CartaPoker(num, palo, tapada)
...
def test_generador_cartas():
    ...
    print("\nFuncion generadora\n",','.join([str(carta) for carta in generador_cartas(25)]))
